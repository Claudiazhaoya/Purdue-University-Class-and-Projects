...
#include"symbol.h"
	...
	%token <num_val> NUM
	%token ASSIGN VAR IF ELSE PLUS MINUS MULTIPLICATION DIVISON EQUAL WRITING NEWLINE SEMI TOP END LEFTPAREN RIGHTPAREN COMMA LEFTCURLY RIGHTCURLY COLON PERIOD OTHER
	%union{
		char *string_val;
		int num_val;
	}
...
;
ifelse:
	IF condition  LEFTCURLY NEWLINE state linestop RIGHTCURLY
	|IF condition LEFTCURLY NEWLINE state linestop RIGHTCURLY ELSE LEFTCURLY NEWLINE state linestop RIGHTCURLY 
	;
condition:
...
if(struct table* ts = symbolTable[$1]){
	...
	}else if(tes==0){//vaerrPrinthas no value and can't be used in expression
	...
	if(struct table* ot = symbolTable[$1]){
		...
		if(op==3){
			if(struct table* ts = symbolTable[temp]){
				...
				sprintf(globalString, "Line %d,%s.%s has no value", line__num+1,$1,$3);
				yyerror(globalString);
			}
		}else{
			sprintf(globalString, "Line %d, type violation", line__num+1);
