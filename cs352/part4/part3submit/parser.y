...
#include"symbol.h"
	...
	%token <num_val> NUM
	%token ASSIGN VAR IF ELSE WHILE FOR DO PLUS MINUS BOOLOR BOOLAND NOT BOOLEQUAL LESSTHAN GREATERTHAN MULTIPLICATION DIVISON EQUAL WRITING NEWLINE SEMI TOP END LEFTPAREN RIGHTPAREN COMMA LEFTCURLY RIGHTCURLY LEFTBRACE RIGHTBRACE COLON PERIOD OTHER
	%union{
		char *string_val;
		int num_val;
	}
...
|ifelse
|WHILE condition LEFTCURLY NEWLINE state NEWLINE RIGHTCURLY linestop
|DO LEFTCURLY NEWLINE state NEWLINE RIGHTCURLY NEWLINE WHILE condition SEMI linestop
...
|ID EQUAL expression linestop{struct table* s=(struct table*)malloc(sizeof(struct table));
	...
	if(struct table* ts = symbolTable[$1]){
		...
		if(!errPrint){
			errPrint=true;
		sprintf(globalString, "Line %d, %s undeclared", line__num+1,$1);
...
|ID LEFTBRACE expression RIGHTBRACE EQUAL expression linestop /* not tested yet*/{
	
}
|NEWLINE{line__num ++;}
;
ifelse:
IF condition  LEFTCURLY todo RIGHTCURLY after/*find just an if statment*/
	{curr_nums.pop();
	curr_types.pop();
	}
...
;
after:
	ELSE LEFTCURLY todo RIGHTCURLY after
	|ELSE ifelse
	|
	;
condition:
...
|VAR ID EQUAL LEFTBRACE array RIGHTBRACE
{arrayid = strdup($2);
	struct table* s=(struct table*)malloc(sizeof(struct table));
	s->type=4;
	symbolTable[$2]=s;
...
|blank expression blank COMMA array
|
;
blank:
NEWLINE blank
...
{char * idtemp = strdup($1);
	...
	if(struct table* ts = symbolTable[$1]){
		...
		if(tes==3){ //object is not allowed as variable
			...
			if(!errPrint){
				errPrint=true;
				fprintf(stderr, "Line %d, %s has no value", line__num+1,$1);
				yyerror(globalString); 
...
|ID LEFTBRACE expression RIGHTBRACE /*not tested yet*/
