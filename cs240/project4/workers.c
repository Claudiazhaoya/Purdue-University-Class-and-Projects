...
void destroy_sharedmem(void* sh_mem, int shm_id) {
...
//child proccess
	if(pid[splitnum]=FORK()==0){
		shared_mem = shmat(shm_id, NULL, 0);
		struct proc_key_count* info = (struct proc_key_count*) shared_mem;
		
		info=info+(splitnum*wordnum);
...
test[testpostion]='\0';
					...
					wordtest[wordplace]='\0';
						wordplace=-1;
							while(testnum<wordnum){
								...
								[testnum])==0){
								sem_wait(&pencil);
								(info+testnum)->pid=getpid();
								(info+testnum)->key_id=testnum;
								(info+testnum)->freq+=1;
								count++;
								sem_post(&pencil);
									}
	...
	sem_wait(&pencil);
	int printercount=0;
	while(printercount<(MAX_PROCS*wordnum)){
			wait((info+printercount)->pid);
			printercount++;
		}
	int printcount=0;
	printercount=0;
	int total=0;
	while(printcount<wordnum){
		while(printercount<(MAX_PROCS*wordnum)){
			if(printcount==(info+printercount)->key_id){
				total+=(info+printercount)->freq;
				}
				printercount++;
			}		
	fprintf(output,"%s: ",keywords[printcount]);
	fprintf(output,"%d\n",total);
	total=0;
	printercount=0;
	printcount++;
	...
	sem_post(&pencil);
	destroy_sharedmem(shared_mem, shm_id);
	fclose(output);
	fclose(input);
	fclose(keyword);
	return 0;
